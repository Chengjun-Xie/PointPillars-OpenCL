...
struct Anchor {
   ...
   */
  Anchor(float _x, float _y, float _z, float _dz)
      : x(_x), y(_y), z(_z), dz(_dz){};
...
struct AnchorGridConfig {
  ...
  std::vector<float> rotations = {0.f,
                                  M_PI_2};  // The set of rotations to in which
                                            // the anchors should be generated
};

struct PointPillarsConfig {
  std::string pfe_model_file{"pfe.onnx"};
  std::string rpn_model_file{"rpn.onnx"};
  std::string opencl_kernel_path{"kernel/"};
  std::string pfe_cache_path{"kernel/"};
  std::string rpn_cache_path{"kernel/"};
  ...
  float rpn_scale{0.5f};  // The scaling factor that the RPN is applying = 1 /
                          // (final convolution stride)
